...
public class AjouterTrajetController implements Initializable {
    ...
    private void chargerVoitures() {
        ...
        if (!voituresList.isEmpty()) {
            cmbVoiture.setValue(voituresList.get(0));
        }
    }
    ...
    private void handleEnregistrer() {
        if (validerFormulaire()) {
            try {
                ...
                int conducteurId = Integer.parseInt(txtConducteurId.getText().trim());
                int voitureId = cmbVoiture.getValue().getId();
                String statut = cmbStatut.getValue();

                // Créer un nouveau trajet (id = 0 car il sera généré par la base de données)
                Trajet nouveauTrajet = new Trajet(0, villeDepart, villeArrivee, dateHeureDepart,
                        nbPlaces, prix, conducteurId, voitureId);
    ...
    private boolean validerFormulaire() {
        ...
        if (txtConducteurId.getText().trim().isEmpty()) {
            erreurs.append("- L'ID du conducteur est requis\n");
        } else {
            try {
                Integer.parseInt(txtConducteurId.getText().trim());
            } catch (NumberFormatException e) {
                erreurs.append("- L'ID du conducteur doit être un nombre\n");
            }
        }
